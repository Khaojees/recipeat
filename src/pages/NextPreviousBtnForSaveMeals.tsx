...
const NextPreviousBtnForSaveMeals = () => {
      ...
      const nextItem = ()=>{
            for(let i=0 ; i<saveMealsItem.length ; i++){
              if(currentMeal?.id === saveMealsItem[i].id){
                if(i+1 === saveMealsItem.length){
                  // console.log("nextItem  = ",i)
                  return saveMealsItem[0]
                }else{
                  // console.log("nextItem  = ",i)
                  return saveMealsItem[i+1]
                }
              }
            }
          }
            ...
            for(let i=0 ; i<saveMealsItem.length ; i++){
              if(currentMeal?.id === saveMealsItem[i].id){
                if(i-1 === -1){
                  ...
                  return saveMealsItem[saveMealsItem.length-1]
                }else{
                  // console.log("previousItem  = ",i)
                  return saveMealsItem[i-1]
                }
...
export default NextPreviousBtnForSaveMeals